...
class PendulumMid : MonoBehaviour
{
    ...
    void UpdatePosition()
    {
        ...
        float thetaTmp = thetaLast + omegaLast * Time.deltaTime * step / 2;
		float omegaTmp = omegaLast - step * Time.deltaTime * g * Mathf.Sin(thetaLast) / length / 2;
        // 3. update theta
		float deltaTheta = omegaTmp * Time.deltaTime * step;
        theta = thetaLast + deltaTheta;
        // 4. update omega
        float deltaOmega = g * Mathf.Sin(thetaTmp) / length * Time.deltaTime * step;
